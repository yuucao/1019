...
library(readr)
BRCA <- read_csv("D:/22fall/INFO550/Final/BRCA.csv",show_col_types = FALSE)
head(BRCA)
```
...
Use boxplots to visualize Protein1-Protein4 distribution in different tumour stages by age category. The cutoff point of age is 58.9, which is the mean of this sample.Use t-test to compare the means of two age groups in different tumour stages. The plots suggest that there is no significance evidence that the means of Protein1-Protein4  are different by tumour stages.
